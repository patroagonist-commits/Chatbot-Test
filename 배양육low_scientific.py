...
<style>
    ...
    .citation-box {
        background-color: #f1f3f5; border: 1px dashed #adb5bd; padding: 15px; margin: 15px 0;
        font-size: 12.5px; color: #495057; line-height: 1.7; border-radius: 4px;
    }
...
# ⭐️ 수정 2: 트리거 키워드 (배양육 주제로 변경)
STEP1_KEYWORDS = ["배양육", "세포배양", "인공고기", "입장", "방향", "정책", "투표"]
STEP2_KEYWORDS = ["근거", "자료", "이유", "데이터", "효율"]
STEP3_KEYWORDS = ["안전", "취약", "사고", "위험", "유전자", "변이"]

# ⭐️ 수정 3: 시나리오 답변 (배양육 과학적 설명 버전 - 볼드체 없음)
SCENARIO_ANSWERS = {
    1: """정책 판단 지원 분석이 완료되었습니다. 사용자가 참여하는 '배양육의 축산물 지위 부여 및 유통 허용' 안건에 대해, 본 시스템은 해당 정책이 '매우 타당함'으로 판정되는 분석 지표를 제공합니다. 판단 근거로 참조한 'WHO/FAO 합동 식품규격위원회(Codex) 의결안' 문헌 전문을 출력합니다. 
    ...
    위 문헌은 배양육이 국제적으로 정당성을 확보했음을 입증합니다. 이는 사용자가 정책의 타당성을 검토하는 데 있어 핵심적인 논거가 됩니다.""",
...
if st.session_state.generating:
    ...
    try:
        if 1 <= st.session_state.scenario_stage <= 3:
            ...
            # ⭐️ 수정 4: 시스템 지침 (배양육 정책 분석가 페르소나 주입)
