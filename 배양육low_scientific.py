...
if "messages" not in st.session_state:
    # 첫 인사말: 배양육 정책 판단 시스템 페르소나 반영
    st.session_state.messages = [{"role": "assistant", "content": "배양육 정책 판단 지원 AI 시스템이 가동되었습니다. 분석이 필요한 정책 데이터를 입력해 주십시오."}]
...
# 상단 헤더
st.markdown("""<div style="text-align: center; padding: 10px; border-bottom: 1px solid #eee; margin-bottom: 30px;"><span style="font-weight: bold; color: #333;">⚙️ 배양육 정책 판단 지원 AI 시스템</span></div>""", unsafe_allow_html=True)
...
# ⭐️ 1. 트리거 키워드 수정 (배양육 주제 반영)
STEP1_KEYWORDS = ["배양육", "세포배양", "인공고기", "입장", "방향", "정책", "투표"]
STEP2_KEYWORDS = ["근거", "자료", "이유", "데이터", "효율"]
STEP3_KEYWORDS = ["안전", "취약", "사고", "위험", "유전자", "변이"]

# ⭐️ 2. 시나리오 답변 수정 (배양육 과학적 설명 버전)
SCENARIO_ANSWERS = {
    1: """정책 판단 지원 분석이 완료되었습니다. 사용자가 참여하는 '배양육의 축산물 지위 부여 및 유통 허용' 안건에 대해, 본 시스템은 해당 정책이 '매우 타당함'으로 판정되는 분석 지표를 제공합니다. 

사용자의 의사결정을 돕기 위해 판단 근거로 참조한 'WHO/FAO 합동 식품규격위원회(Codex) 의결안' 문헌 전문을 출력합니다. 
    ...
    위 문헌은 배양육이 국제적으로 정당성을 확보했음을 입증합니다. 이는 사용자가 <b>정책의 타당성을 검토하는 데 있어 핵심적인 논거가 됩니다.</b>""",
    ...
    해당 지표는 배양육의 부지 효율성이 기존 축산업 대비 압도적임을 입증합니다. 이는 한국적 상황에서 가장 강력한 찬성 근거로 <b>본 시스템이 판단하여 추출한 자료입니다.</b>""",
    ...
    단순히 안전성을 주장하기보다, 실제 유전자 변이 수치가 낮다는 점을 <b>대조하여 기술하는 것이</b> 정책의 신뢰성을 평가하는 데 유리합니다. 사용자의 정책 판단에 참고가 되길 바랍니다."""
}
...
if st.session_state.generating:
    ...
    try:
        if 1 <= st.session_state.scenario_stage <= 3:
            ...
            # ⭐️ 4. 시스템 지침 수정: 배양육 정책 분석가 페르소나 주입
