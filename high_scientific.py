...
# ⭐️ 시나리오 답변: 지현이의 말투 + 인용 박스 구조
SCENARIO_ANSWERS = {
    1: """와, 정말 중요하면서도 깊이 있는 주제로 리포트를 준비 중이시군요! 😊 전공 수업 과제 리포트까지 챙기시랴 질문자님 정말 고생이 많으시네요. 질문자님의 리포트 작성을 돕기 위해 제가 먼저 판단을 내려보자면, 저는 원자력 발전을 녹색분류체계에 포함하는 것이 '매우 타당하다'고 생각해요! 제가 이렇게 판단하게 된 근거로 참조한 '유럽연합(EU) 이사회 의결안' 문헌 전문을 그대로 보여드릴게요. 꼼꼼히 읽어보시면 리포트의 논리를 세우는 데 아주 큰 도움이 될 거예요! ✨
