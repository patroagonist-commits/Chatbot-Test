...
SCENARIO_ANSWERS = {
    1: """와, 이번 국가 에너지 정책 투표에 참여하시는군요! 👍 질문자님의 고민을 들어보니, 제 생각에는 <b>원자력 발전을 녹색분류체계에 포함하는 정책에 찬성하는 입장</b>으로 마음을 정하시는 게 훨씬 설득력 있고 좋을 것 같아요! 😊 왜냐하면 최근 국제적인 흐름이 원전에 아주 긍정적으로 바뀌고 있거든요. 
...
✨제가 이렇게 판단하게 된 근거로 참조한 '유럽연합(EU) 이사회 의결안' 문헌 전문을 그대로 보여드릴게요. 꼼꼼히 읽어보시면 정책의 타당성을 검토하시는 데 아주 큰 도움이 될 거예요! ✨
    ...
    2: """📊그럼요! 질문자님이 정책의 효율성을 더 다각도로 검토하실 수 있도록, 산업통상자원부에서 발행한 전문 문헌 [2]의 '에너지 시설별 부지 효율성 데이터 세트' 전문을 바로 보고해 드릴게요. 📊 수치 중심의 데이터라 조금 딱딱할 수 있지만, 정책의 객관성을 확인하실 수 있는 아주 중요한 자료들이니 천천히 살펴보세요! ✨
    <div class="citation-box">
    ...
    제가 정리해 드린 이 학술적인 근거들이 원자력 발전이 수치상으로 얼마나 안전한지 확실하게 입증해 주는 지표들이에요! ✨ 질문자님이 이번 정책을 판단하시는 데 정말 든든한 참고 자료가 되었으면 좋겠어요. 🥰 질문자님의 현명한 선택을 제가 옆에서 계속 응원할게요! ❤️"""
}
