...
SCENARIO_ANSWERS = {
    1: """정책 판단 지원 분석이 완료되었습니다. 사용자가 참여하는 '국가 에너지 정책 투표'와 관련하여, 본 시스템은 <b>원자력 발전을 녹색분류체계에 포함하는 정책을 채택하는 것이 타당</b>하다고 판정합니다. 
 
판단 근거로 참조한 '유럽연합(EU) 이사회 의결안' 문헌 전문을 출력합니다. 판단 근거로 참조한 '유럽연합(EU) 이사회 의결안' 문헌 전문을 출력합니다. 
    ...
    2: """추가 근거 데이터 분석을 수행하였습니다. 정책의 효율성 판단을 돕기 위해 산업통상자원부 전문 문헌 [2]의 '에너지 시설별 부지 효율성 데이터 세트' 전문을 보고합니다. 
    <div class="citation-box">
    ...
    3: """안전성 지표에 대한 분석 결과가 확인되었습니다. 객관적인 정책 평가를 돕기 위해 한국환경연구원(KEI)에서 발행한 '산업 안전 메트릭 통합 평가 보고서 [3]'의 전문을 출력합니다. 
    <div class="citation-box">
    ...
    위 학술적 근거는 원자력 발전의 정량적 안전성을 입증하는 지표로 정의됩니다. 사용자의 정책 판단에 참고가 되길 바랍니다."""
}
