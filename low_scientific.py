...
# ⭐️ 시나리오 답변 (저의인화 X 과학적 설명 버전)
SCENARIO_ANSWERS = {
    1: """입장 추천 분석이 완료되었습니다. 사용자가 제시한 리포트 주제에 대해, 본 시스템은 원자력 발전을 녹색분류체계에 포함하는 것이 '매우 타당함'으로 판정합니다. 

해당 입장을 뒷받침하기 위해 수많은 정책 지표 중 <b>가장 결정적인 근거를 선별하여 제시합니다.</b> 판단 근거로 참조한 '유럽연합(EU) 이사회 의결안' 문헌 전문을 출력합니다. 
    ...
    2: """추가 근거 데이터 분석을 수행하였습니다. 리포트의 설득력을 높이기 위해 태양광 발전 대비 원전이 가진 <b>차별적 우위를 대조하여 설명합니다.</b> 산업통상자원부에서 발행한 전문 문헌 [2]의 '에너지 시설별 부지 효율성 데이터 세트' 전문을 보고합니다. 
    <div class="citation-box">
